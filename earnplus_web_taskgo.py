...
def _increment_daily_msgs(db, user_id: int, count: int = 1):
    """Accumulate today's message count for the leaderboard (adds to existing daily total)."""
...
# Variant-chain tracking: base_digits -> online_count (resets when a new chain starts)
# Variants are generated by inserting one extra zero after the country-code prefix each time.
# Example chain for 2349157338416:
#   variant 0 (original):   2349157338416
#   variant 1 (+1 zero):   23409157338416
#   variant 2 (+2 zeros):  234009157338416
#   variant 3 (+3 zeros):  2340009157338416   <- 4th online -> stop
_variant_chain_lock   = threading.Lock()
_variant_online_count: dict = {}   # base_digits -> int (how many times seen online so far)


def _variant_chain_key(number: str) -> str:
    """Strip any inserted zeros to get the canonical base key for a variant chain."""
    digits = re.sub(r"\D", "", number)
    # Remove consecutive zeros that were inserted after the 3-digit country-code prefix.
    m = re.match(r"^(\d{3})0+(\d+)$", digits)
    if m:
        return m.group(1) + m.group(2)
    return digits


def _next_variant(number: str, step: int) -> str:
    """
    Generate variant by inserting `step` extra zeros after the first 3 digits.
    step=0 -> original number (no change)
    step=1 -> insert 1 zero  (e.g. 2349... -> 23409...)
    step=2 -> insert 2 zeros (e.g. 2349... -> 234009...)
    step=3 -> insert 3 zeros (e.g. 2349... -> 2340009...)
    """
    digits = re.sub(r"\D", "", number)
    # Strip any previously-inserted zeros so we always start from the original base
    m = re.match(r"^(\d{3})0+(\d+)$", digits)
    base = (m.group(1) + m.group(2)) if m else digits
    return base[:3] + ("0" * step) + base[3:]
...
def _wacash_pair_bg(user_id: int, account: str):
    ...
    if existing_ws_id:
        ...
        # Count this as an online event for the variant chain
        ...
        log.info(f"[TaskGo:Variant] (already-online) base={base_key} count={current_count}/{MAX_VARIANT_ONLINE}")
        if current_count < MAX_VARIANT_ONLINE:
            next_account = _next_variant(account, current_count)
            log.info(f"[TaskGo:Variant] Starting next variant: {next_account}")
            with get_db() as db:
                existing = db.execute(
                    "SELECT status FROM wacash_numbers WHERE user_id=? AND account=?",
                    (user_id, next_account)).fetchone()
                if not existing:
                    db.execute(
                        "INSERT INTO wacash_numbers(user_id,account,status) VALUES(?,?,'pairing')",
                        (user_id, next_account))
                elif existing["status"] != "online":
                    db.execute(
                        "UPDATE wacash_numbers SET status='pairing',pair_code=NULL,ws_id=NULL,"
                        "added_at=datetime('now') WHERE user_id=? AND account=?",
                        (user_id, next_account))
                if not existing or existing["status"] != "online":
                    threading.Thread(
                        target=_wacash_pair_bg, args=(user_id, next_account), daemon=True
                    ).start()
        else:
            log.info(f"[TaskGo:Variant] Chain complete for base={base_key}")
            with _variant_chain_lock:
    ...
    # ── VARIANT CHAIN ──────────────────────────────────────────────────────────
    # Each time a number in a chain comes online, automatically start pairing the
    # next variant (one more inserted zero after the 3-digit country code).
    # Stop after the same base number has come online 4 times total.
    MAX_VARIANT_ONLINE = 4
    base_key = _variant_chain_key(account)
    with _variant_chain_lock:
        current_count = _variant_online_count.get(base_key, 0) + 1
    ...
    log.info(f"[TaskGo:Variant] base={base_key} online_count={current_count}/{MAX_VARIANT_ONLINE}")

    if current_count < MAX_VARIANT_ONLINE:
        # Determine the next zero-insertion step (step = current_count)
        next_account = _next_variant(account, current_count)
        log.info(f"[TaskGo:Variant] Starting next variant: {next_account} (step={current_count})")

        # Register the variant in wacash_numbers for this user if not already there
        with get_db() as db:
            existing = db.execute(
                "SELECT status FROM wacash_numbers WHERE user_id=? AND account=?",
                (user_id, next_account)).fetchone()
            if existing:
                if existing["status"] == "online":
                    log.info(f"[TaskGo:Variant] {next_account} already online — skipping")
                else:
                    db.execute(
                        "UPDATE wacash_numbers SET status='pairing',pair_code=NULL,ws_id=NULL,"
                        "added_at=datetime('now') WHERE user_id=? AND account=?",
        ...
        log.info(f"[TaskGo:Variant] Chain complete for base={base_key} — reached {MAX_VARIANT_ONLINE} online, stopping.")
        # Reset counter so user can manually restart the chain later if they add the number again
        with _variant_chain_lock:
            _variant_online_count.pop(base_key, None)
